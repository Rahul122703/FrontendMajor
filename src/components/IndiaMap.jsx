...
const HoverCard = ({ point, position, isVisible }) => {
  ...
  return (
    ...
    >
      <h3 className="font-bold text-sm mb-2 text-gray-900">{point.region_name || 'Unknown Region'}</h3>
      <div className="space-y-1 text-xs">
        <div className="flex justify-between">
          <span className="text-gray-600">Temperature:</span>
          <span className="font-semibold">{temp !== null ? Math.round(temp) + '°C' : 'N/A'}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-600">Heatwave Risk:</span>
          <span className={`font-semibold ${
            hwProb >= 0.6 ? 'text-red-600' : 
            hwProb >= 0.4 ? 'text-orange-500' : 
            hwProb >= 0.2 ? 'text-yellow-500' : 'text-green-500'
          }`}>
        ...
        <div className="flex justify-between">
          ...
          <span className="font-semibold">{new Date(point.forecast_date).toLocaleDateString()}</span>
        </div>
        <div className="flex justify-between">
...
const IndiaMap = ({ data, selectedPoint, onPointClick }) => {
  ...
  return (
    <div className={`relative bg-white rounded-lg border border-gray-200 overflow-hidden ${isFullscreen ? 'fixed inset-0 z-50 rounded-none' : ''}`}>
      ...
      <div className={`absolute bottom-4 left-4 bg-white p-3 rounded-lg shadow-lg border border-gray-200 z-1000 ${isFullscreen ? 'scale-110' : ''}`}>
      ...
      <div className={`absolute bottom-4 right-4 bg-white p-3 rounded-lg shadow-lg border border-gray-200 z-1000 ${isFullscreen ? 'scale-110' : ''}`}>
